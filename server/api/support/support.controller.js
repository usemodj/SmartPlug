 ...
 * GET     /api/supports              ->  index
 * POST    /api/supports              ->  create
 * GET     /api/supports/:id          ->  show
 * PUT     /api/supports/:id          ->  update
 * DELETE  /api/supports/:id          ->  destroy
 */
...
export function tagSupports(req, res, next){
  ...
  Taggable.countAsync({tag: tag, type: 'Support'})
    ...
    .then(taggables => {
      ...
      return Support.findAsync({_id: {$in: ids}, 'author.email': user.email})
    })
    .then(supports => {
      res.status(200).json(supports);
    })
