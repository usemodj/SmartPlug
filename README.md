...
2. [Node.js](http://nodejs.org) Node 9.x/8.x :
...
6. [Grunt](http://gruntjs.com/) (`yarn global add grunt-cli`)
...
1. Run `yarn` or `yarn install` to install all dependencies for this project.
The dependencies are retrieved from your project’s `package.json` file,
and stored in the `yarn.lock` file:
```
$ yarn
```

 Upgrades packages to their latest version
based on the version range specified in the `package.json` file:
```
$ yarn upgrade
$
...
>
> [mongodb/js-bson](https://github.com/mongodb/js-bson) :
...
## Build & development
Run `grunt build` for building the distribution into `dist/` directory
...
* Install Elastic Search Server and mongoosastic search pagination :
 [./docs/install-elasticsearch.md](docs/install-elasticsearch.md)
